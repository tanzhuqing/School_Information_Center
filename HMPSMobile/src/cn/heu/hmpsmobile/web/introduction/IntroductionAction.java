...
public class IntroductionAction extends BaseAction
{
	...
	// 组织机构子页面 由索引num查询对应对象
	public String lisOrganizationSubDetailJSON()
	{

		//String num = this.organSub.getNum();
		this.organSubDetaillist = this.introductionService.findByField(this.num);
		JSONArray jsonArray = new JSONArray();
        for (OrganizationSub organSub : organSubDetaillist) {
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("id", organSub.getId());
            jsonObject.put("tId", organSub.getTid());
            jsonObject.put("child", organSub.getChild());
	...
	private List<OrganizationSub> organSubDetaillist; 
